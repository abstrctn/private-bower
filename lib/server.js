...
function _loadConfig() {
    ...
    _config.socket = _config.socket || false;
...
function _initService() {
    ...
    if (_config.socket) {
        var server = app.listen(_config.socket, function() {
            fs.chmodSync(_config.socket, '777');
            logger.log('Bower server started on port ' + _config.port);
        });
        server.on('error', function(e) {
            if (e.code == 'EADDRINUSE') {
                fs.unlink(_config.socket);
                server.listen(_config.socket, function() {
                    fs.chmodSync(_config.socket, '777');
                    logger.log('Server recovered from socket connection failure.');
                });
            }
        });
    } else {
